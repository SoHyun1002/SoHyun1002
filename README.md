# 👩‍💻 Sohyun Choi

## 🌐 Full-Stack Web Developer
실무 중심의 풀스택 웹 개발자로,  
프론트엔드부터 백엔드, 배포·운영까지 전반적인 웹 서비스 개발을 경험하고 있습니다.

- **현업 풀스택 웹 개발자 재직 중 (2025.07.07 ~ )**
- JavaScript 기반 프론트엔드 & 백엔드 개발
- Vue.js + Quasar Framework + Pinia 기반 SPA 개발
- Spring Boot 기반 백엔드 및 MSA 구조 프로젝트 경험
- CI/CD 파이프라인 구축 및 자동 배포 환경 구성 경험

---

## 💼 Work Experience

### 🏢 Full-Stack Web Developer  
**2025.07.07 ~ Present**

- JavaScript 기반 **풀스택 웹 개발**
- Vue.js + Quasar Framework + Pinia 를 활용한 프론트엔드 개발
- **사내 핵심 시스템(Tracker, ERP) 개발 및 기여**
- 내부 업무용 웹 서비스의 개발, 유지보수 및 운영
- Local Nginx 기반 개발/운영 환경 구성
- MariaDB 연동 및 데이터베이스 활용
- Jenkins 기반 CI/CD 파이프라인 구축 및 배포 자동화

---

## 🤖 Python 업무자동화 & AI 활용
- Python 기반 업무 자동화 및 데이터 처리
- OpenAI API를 활용한 프롬프트 엔지니어링 경험
- 반복 업무 효율화를 위한 자동화 도구 개발

---

## 📝 Documentation & Communication
- 기술 문서, API 문서, 설계서 등 문서화 경험
- 개발 과정과 의사결정 기록에 익숙
- 팀 내 협업 및 지식 공유에 강점

---

## 🛠️ Tech Stack

### 💻 Language
![Java](https://img.shields.io/badge/Java-007396?style=flat&logo=java&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)

### 🎨 Frontend
![Vue.js](https://img.shields.io/badge/Vue.js-4FC08D?style=flat&logo=vue.js&logoColor=white)
![Quasar](https://img.shields.io/badge/Quasar-1976D2?style=flat&logo=quasar&logoColor=white)
![Pinia](https://img.shields.io/badge/Pinia-FFD859?style=flat&logo=vue.js&logoColor=black)
![React](https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)

### ⚙️ Backend
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=spring-boot&logoColor=white)
![JPA](https://img.shields.io/badge/JPA-59666C?style=flat&logo=hibernate&logoColor=white)
![JWT](https://img.shields.io/badge/JWT-000000?style=flat&logo=JSON%20web%20tokens&logoColor=white)

### 🗄️ Database
![MariaDB](https://img.shields.io/badge/MariaDB-003545?style=flat&logo=mariadb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=flat&logo=redis&logoColor=white)

### 🚀 DevOps & Infra
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=flat&logo=nginx&logoColor=white)
![Jenkins](https://img.shields.io/badge/Jenkins-D24939?style=flat&logo=jenkins&logoColor=white)

---

## 📈 GitHub Stats

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=SoHyun1002&layout=compact)
![Sohyun's GitHub stats](https://github-readme-stats.vercel.app/api?username=SoHyun1002&show_icons=true)

---

## 📫 Contact
- 📧 Gmail : **Theld2403@gmail.com**
- 🐙 GitHub : https://github.com/SoHyun1002
